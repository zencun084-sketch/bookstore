<template>
  <div class="search-page container">
    <div class="search-header">
      <h2>搜索结果</h2>
      <p v-if="keyword">关键词：<b>{{ keyword }}</b>，共找到 {{ total }} 本图书</p>
    </div>

    <div class="filter-bar">
      <div class="sort-tabs">
        <span :class="['sort-tab', sortField === 'sales' && sortOrder === 'desc' ? 'active' : '']" @click="changeSort('sales', 'desc')">销量</span>
        <span :class="['sort-tab', sortField === 'create_time' && sortOrder === 'desc' ? 'active' : '']" @click="changeSort('create_time', 'desc')">最新</span>
        <span :class="['sort-tab', sortField === 'price' && sortOrder === 'asc' ? 'active' : '']" @click="changeSort('price', 'asc')">价格↑</span>
        <span :class="['sort-tab', sortField === 'price' && sortOrder === 'desc' ? 'active' : '']" @click="changeSort('price', 'desc')">价格↓</span>
      </div>
      <div class="price-filter">
        <el-input v-model="minPrice" placeholder="最低价" size="small" style="width: 100px" />
        <span>-</span>
        <el-input v-model="maxPrice" placeholder="最高价" size="small" style="width: 100px" />
        <el-button size="small" @click="fetchBooks">筛选</el-button>
      </div>
    </div>
    ...
    <div class="pagination" v-if="total > 0">
      <el-pagination v-model:current-page="current" :page-size="12" :total="total" layout="prev, pager, next, total" @current-change="fetchBooks" />
    </div>
...
.sort-tab { font-size: 14px; color: var(--text-secondary); cursor: pointer; padding: 4px 0; position: relative; }
.sort-tab.active { color: var(--primary-color); font-weight: 600; }
.sort-tab.active::after { content: ''; position: absolute; bottom: -4px; left: 0; right: 0; height: 2px; background: var(--primary-color); }
.price-filter { display: flex; align-items: center; gap: 8px; }

.book-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; min-height: 200px; }
.book-card { background: #fff; border-radius: var(--radius-md); overflow: hidden; cursor: pointer; transition: all 0.2s; box-shadow: var(--shadow-sm); }
.book-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-lg); }
.book-cover { height: 200px; background: #f3f4f6; display: flex; align-items: center; justify-content: center; overflow: hidden; }
